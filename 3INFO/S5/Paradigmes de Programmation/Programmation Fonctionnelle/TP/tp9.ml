...
let rec eval (fm : formula) (v : valuation) : bool = match fm with
  | True -> true
  | False -> false
  | P(s) -> (get_value s v)
  | Not(f) -> not (eval f v)
...
let atoms (fm : formula) : string list =
  let rec atoms_rec fm l = match fm with
    | True | False -> l
    | Not(f) -> atoms_rec f l
    | And(f1, f2) -> atoms_rec f1 (atoms_rec f2 l)
    | Or(f1, f2) -> atoms_rec f1 (atoms_rec f2 l)
    | Imp(f1, f2) -> atoms_rec f1 (atoms_rec f2 l)
    | Iff(f1, f2) -> atoms_rec f1 (atoms_rec f2 l)
    | P(s) -> if (List.exists (fun x -> x = s) l) then l else s::l
  in atoms_rec fm [];;
...
let tautology (fm : formula) : bool =
  let rec insert elt ll = match ll with
    | [] -> []
    | a::reste -> ((elt, true)::a)::((elt, false)::a)::(insert elt reste)
  in let rec all_possibilities atms ll = match atms with
       | [] -> ll
       | a::reste -> all_possibilities reste (insert a ll)
  in not (List.exists (fun v -> not (eval fm v)) (all_possibilities (atoms fm) [[]]));;
...
let find_truth (f : valuation -> unit) (fm : formula) : unit =
  let rec insert elt ll = match ll with
    | [] -> []
    | a::reste -> ((elt, true)::a)::((elt, false)::a)::(insert elt reste)
  in let rec all_valuations atms ll = match atms with
       | [] -> ll
       | a::reste -> all_valuations reste (insert a ll)
  in let rec eval_all valuations = match valuations with
       | [] -> ()
       | v::reste -> if eval fm v then (f v) else (); eval_all reste
  in eval_all (all_valuations (atoms fm) [[]])
...
let rec nnf : formula -> formula =
  fun fm -> match fm with
            | True | False | P(_) -> fm
            | Imp(f1, f2) -> Or(nnf (Not(f1)), nnf f2) 
            | Iff(f1, f2) -> Or(And(nnf f1, nnf f2), And(nnf (Not(f1)), nnf (Not(f2))))
            ...
            | Not(f) -> match f with
                        ...
                        | Iff(p, q) -> Or(And(nnf p, nnf (Not(q))), And(nnf q, nnf (Not(p))))
                        | True -> False
                        | False -> True
                        | P(_) -> fm;;
...
let simplify formula =
  let open List in
  let setify l = sort_uniq compare l in
  let union =
    let rec union l1 l2 =
      match l1, l2 with
      | [], l2 -> l2
...
let _ = find_truth (print_valuation) (string_to_formula "A1 <=> (~A1 . ~A2)");;

(* If I am a knight there is gold on the island.
   Can you tell the type of the native and if there is gold
   on the island?
   k : the native is a knight
   g : there is gold on the island
 *)

let _ = find_truth (print_valuation) (string_to_formula "k <=> (k => g)");;
...
(*
  ...
  s : there is silver on the island
 *)

let _ = find_truth (print_valuation) (string_to_formula "(k <=> g) . (k <=> (g => s))");;
...
let _ = find_truth (print_valuation) (string_to_formula "(A1 <=> ((A1 . A2) => g)) . (A2 <=> A1)");;
...
(*
  ...
  i : the defendant is innocent
 *)

let _ = find_truth (print_valuation) (string_to_formula "(A1 <=> (A2 => i)) . (A2 <=> ((~A1) => i))");;
...
let _ = find_truth (print_valuation) (string_to_formula "(A1 <=> ((A1 + A2) => g)) . (A2 <=> ((~A1 + ~A2) => g))");;
...
(* test : 
   let _ = find_truth (print_valuation) (string_to_formula "(~(a => b)) <=> (a . ~b)");;
 *)
...
let _ = find_truth (print_valuation) (string_to_formula "(f1 <=> ~f2) . (((f1.~k1)+(k1.~f1)) <=> f2.k2) . (((f2.~k2)+(k2.~f2)) <=> k1.k2)");;


(*
  If this sentence is true then Santa Claus exists.
 *)
let _ = find_truth (print_valuation) (string_to_formula "sentance <=> (sentance => santa)");;
