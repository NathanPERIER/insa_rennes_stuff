...
test_selection_lyon :-
        sortall([NumPiece,Nom], selection_lyon(NumPiece,Nom),[[p1,tole], [p2,jante]]).

test_projection :-
        sortall([Nom,Lieu], projection(Nom,Lieu),[[jante, lyon], [jante, marseille], [piston, toulouse], [pneu, clermontFerrand], [soupape, lille], [tole, lyon], [tole, marseille], [vitre, marseille], [vitre, nancy]]).

test_union :-
        sortall([Nom,Ville], union(Nom, Ville),[[brown, marseille], [dupond, lille], [dupont, lyon], [durand, lille], [martin, rennes], [michel, clermontFerrand], [smith, paris]]).
 
test_intersection :-
        sortall([Nom,Ville], intersection(Nom, Ville),[[brown, marseille], [dupont, lyon], [durand, lille], [martin, rennes], [michel, clermontFerrand], [smith, paris]]).

test_difference :-
        sortall([Nom,Ville], difference(Nom, Ville),[[dupond, lille]]).

test_produit_cartesien :-
        sortall([NumF1, Nom, marseille, NumF2, Piece, Quantite], produit_cartesien(NumF1, Nom, marseille, NumF2, Piece, Quantite),[[f6, brown, marseille, f1, p1, 300], [f6, brown, marseille, f1, p2, 300], [f6, brown, marseille, f2, p2, 200], [f6, brown, marseille, f3, p3, 200], [f6, brown, marseille, f4, p1, 300], [f6, brown, marseille, f4, p2, 300], [f6, brown, marseille, f4, p4, 400], [f6, brown, marseille, f6, p5, 500], [f6, brown, marseille, f6, p6, 1000], [f6, brown, marseille, f6, p7, 300]]),
        sortall([NumF1, Nom, lyon, NumF2, Piece, Quantite], produit_cartesien(NumF1, Nom, lyon, NumF2, Piece, Quantite),[[f1, dupont, lyon, f1, p1, 300], [f1, dupont, lyon, f1, p2, 300], [f1, dupont, lyon, f2, p2, 200], [f1, dupont, lyon, f3, p3, 200], [f1, dupont, lyon, f4, p1, 300], [f1, dupont, lyon, f4, p2, 300], [f1, dupont, lyon, f4, p4, 400], [f1, dupont, lyon, f6, p5, 500], [f1, dupont, lyon, f6, p6, 1000], [f1, dupont, lyon, f6, p7, 300]]),
        sortall([NumF1, Nom, rennes, NumF2, Piece, Quantite], produit_cartesien(NumF1, Nom, rennes, NumF2, Piece, Quantite),[[f3, martin, rennes, f1, p1, 300], [f3, martin, rennes, f1, p2, 300], [f3, martin, rennes, f2, p2, 200], [f3, martin, rennes, f3, p3, 200], [f3, martin, rennes, f4, p1, 300], [f3, martin, rennes, f4, p2, 300], [f3, martin, rennes, f4, p4, 400], [f3, martin, rennes, f6, p5, 500], [f3, martin, rennes, f6, p6, 1000], [f3, martin, rennes, f6, p7, 300]]).

test_jointure :-
        sortall([NumF, Nom, Ville, Piece, Quantite], jointure(NumF, Nom, Ville, Piece, Quantite),[[f1, dupont, lyon, p1, 300], [f1, dupont, lyon, p2, 300], [f2, durand, lille, p2, 200], [f3, martin, rennes, p3, 200], [f4, michel, clermontFerrand, p1, 300], [f4, michel, clermontFerrand, p2, 300], [f4, michel, clermontFerrand, p4, 400], [f6, brown, marseille, p5, 500], [f6, brown, marseille, p6, 1000], [f6, brown, marseille, p7, 300]]).

test_jointure_sup :-
        sortall([NumF, Nom, Ville, Piece, Quantite], jointure_sup(NumF, Nom, Ville, Piece, Quantite),[[f4, michel, clermontFerrand, p4, 400], [f6, brown, marseille, p5, 500], [f6, brown, marseille, p6, 1000]]).

test_division :-
        sortall(F, division(F),[f1, f4]).

test_total_pieces_livrees_fournisseur :-
        sortall([NumF,QteLivree], total_pieces_livrees_fournisseur(NumF,QteLivree),[[f1, 600], [f2, 200], [f3, 200], [f4, 1000], [f5, 0], [f6, 1800]]).

test_est_compose_de :-
        sortall(X, est_compose_de(voiture,X),[jante, moteur, piston, pneu, porte, roue, soupape, tole, vitre]),
        sortall(X, est_compose_de(moteur,X),[piston, soupape]),
        not est_compose_de(jante,X).

test_nb_pieces_tot :-
        sortall([Composant,Piece,Qte], nb_pieces_tot(Composant,Piece,Qte),[[moteur, piston, 4], [moteur, soupape, 16], [porte, tole, 1], [porte, vitre, 1], [roue, jante, 1], [roue, pneu, 1], [voiture, jante, 4], [voiture, piston, 4], [voiture, pneu, 4], [voiture, soupape, 16], [voiture, tole, 4], [voiture, vitre, 4]]).

test_nb_voiture :-
        sortall(Nb, nb_voiture(Nb),[62]).
...
produit_cartesien(A, B, C, D, E, F):-
        fournisseurReference(A, B, C),
        livraison(D, E, F).
...
not_all_finds(X):-
        piece(P, _1, lyon),
        not livraison(X, P , _2).

division(X):-
        fournisseurReference(X, _1, _2),
        not not_all_finds(X).
...
total_pieces_livrees_fournisseur(F, N):-
        fournisseurReference(F, _1, _2),
        findall(X, livraison(F, _, X), R),
        sum_all(R, N).
