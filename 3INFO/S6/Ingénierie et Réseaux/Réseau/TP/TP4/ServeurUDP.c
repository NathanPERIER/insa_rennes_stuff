...
int main(int argc, char** argv) {
	...
	while (1) {
		...
		ret = recvfrom(sock, buf_read, 256, 0, (struct sockaddr*) &client, (socklen_t*) &client_len);
		if (ret < 0) {
		...
		ret = sendto(sock, buf_write, strlen(buf_write), 0, (struct sockaddr *) &client, sizeof(client));
		if (ret <= 0) {
			printf("%s: sendto=%d: %s\n", argv[0],ret,strerror(errno));
			continue;
		}
