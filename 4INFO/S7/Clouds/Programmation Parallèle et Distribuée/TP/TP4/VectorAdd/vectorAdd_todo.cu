...
//
void vector_add_c(float* pOut, float* pIn_1, float* pIn_2, int n) {
	for(int i = 0; i < n; i++) {
...
__global__ void vector_add_basic_cuda(float* dOut, float* dIn_1, float* dIn_2, int n) {
	for(int i = 0; i < n; i++) {
		dOut[i] = dIn_1[i] + dIn_2[i];
	}
}

__global__ void vector_add_thread_cuda(float* dOut, float* dIn_1, float* dIn_2, int n) {
	int id = blockIdx.x * blockDim.x + threadIdx.x;
	int increment = gridDim.x * blockDim.x;
	for(unsigned int i = id; i < n; i += increment) {
		dOut[i] = dIn_1[i] + dIn_2[i];
	}
}

__global__ void vector_add_grid_cuda(float* dOut, float* dIn_1, float* dIn_2, int n) {
...
int main() {
	...
	for(int i = 0; i < N; i++) {
		...
		// dIn_1[i] = 1.0f; 
		// dIn_2[i] = 2.0f;
		// Il n'est en effet pas possible d'accéder à de la mémoire dite 'device', 
		// c'est à dire allouée via 'cudaMalloc' directement depuis le CPU...
	...
	// cudaMemcpy(void *dst, void *src, size_t count, cudaMemcpyKind kind) : copie les 'count' octets de 'src' vers 'dst'
	...
	// vector_add_thread_cuda<<<1, 32>>>(dOut, pIn_1, pIn_2, N);
	vector_add_grid_cuda<<<ceil((float)N/256.0), 256>>>(dOut, pIn_1, pIn_2, N);
	...
	// TODO copie GPU vers CPU 
	HANDLE_ERROR(cudaMemcpy(pOut, dOut, sizeof(float) * N, cudaMemcpyDeviceToHost));

	// On affiche la première sortie du résultat :
	std::cout << "First value in CPU buffer : " << pOut[0] << std::endl;
	// La ligne suivante compile aussi mais l'exécution provoque un crash à cause des remarques précédentes...
	// TODO renseigner first_value_on_gpu
	float first_value_on_gpu;
	HANDLE_ERROR(cudaMemcpy(&first_value_on_gpu, dOut, sizeof(float), cudaMemcpyDeviceToHost));
