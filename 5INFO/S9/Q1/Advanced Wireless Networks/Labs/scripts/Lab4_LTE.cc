...
main (int argc, char *argv[])
{
  ...
  uint32_t ueX = 1;
  uint32_t ueY = 0;
  std::string antennaConf = "Isotropic";
  uint32_t payloadSize = 1024;
  std::string dataRate = "50.0Mbps";
  ...
  cmd.AddValue("x", "The x coordinate of the UE (> 0)", ueX);
  cmd.AddValue("y", "The y coordinate of the UE (>= 0)", ueY);
  cmd.AddValue("antenna", "The antenna configuration (Parabolic, Cosine, Isotropic)", antennaConf);
  cmd.AddValue("payloadSize", "The size of the application payload in MB", payloadSize);
  ...
  */
  ...
  lteHelper->SetEnbAntennaModelType ("ns3::" + antennaConf + "AntennaModel");
  ...
  MM -> SetPosition(Vector3D(ueX,ueY,1)); // distance between UE and eNodeB is 5 metres
...
// Install and start applications on UE and remote host
 ...
 uint16_t dlPort = 1000;
  ...
  ApplicationContainer onOffApp;
    ...
    onOffHelper.SetAttribute("DataRate", DataRateValue(DataRate(dataRate))); //Traffic Bit Rate
